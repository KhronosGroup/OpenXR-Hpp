...
By default OpenXR-Hpp has exceptions enabled. This means that OpenXR-Hpp checks the return code of each function call which returns a Xr::Result. If Xr::Result is a failure a std::runtime_error will be thrown. Since there is no need to return the error code anymore the C++ bindings can now return the actual desired return value, i.e. a OpenXR handle. In those cases ResultValue <SomeType>::type is defined as the returned type.
...
All over openxr.hpp, there are a couple of calls to an assert function. By defining OPENXR_HPP_ASSERT, you can specifiy your own custom assert function to be called instead.
...
A client _could_ simply pass `{ instance }` into the `createDebugUtilsMessengerEXT` call instead to implicitly create the dispatch object, but that approach is not recommended as it will trigger discovery and loading of the called function pointer every single time.
